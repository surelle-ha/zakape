...
test.describe('phone workbench', () => {
  ...
  test('keeps drawing, tools, layers, and frames reachable by touch', async ({ page }) => {
    ...
    await applicationMenu.getByRole('button', { name: 'Editor' }).click()
    await expect(applicationMenu.getByRole('menuitem', { name: 'Appearance…' })).toBeVisible()
    await expect(
      applicationMenu.getByRole('menuitem', { name: 'Assistant Settings…' }),
    ).toBeVisible()
    await expect(applicationMenu.getByRole('menuitem', { name: 'Toolbox Editor…' })).toBeVisible()
    await expect(applicationMenu.getByRole('menuitem', { name: 'Undo' })).toHaveCount(0)
    await applicationMenu.getByRole('menuitem', { name: 'Appearance…' }).click()
    const appearanceDialog = page.getByRole('dialog', { name: 'Shape the workbench' })
    const appearanceBounds = await appearanceDialog.boundingBox()
    expect(appearanceBounds!.x).toBeGreaterThanOrEqual(0)
    expect(appearanceBounds!.y).toBeGreaterThanOrEqual(0)
    expect(appearanceBounds!.x + appearanceBounds!.width).toBeLessThanOrEqual(412)
    expect(appearanceBounds!.y + appearanceBounds!.height).toBeLessThanOrEqual(839)
    await appearanceDialog.getByRole('button', { name: 'Cancel' }).click()
    await menuTrigger.click()
