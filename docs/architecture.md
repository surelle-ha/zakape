...
Type checks, unit tests, static generation, Rust formatting/lints, Playwright journeys, and UI snapshots run locally and in GitHub Actions. Interaction coverage includes document tabs, guarded project/application closing, undoable frame reordering, shape previews, frame-local menus, onion skinning, hand panning, zoom-linked work grids, Ctrl-wheel zoom, context-menu suppression, and browser-shortcut hardening. Native release bundles are produced on tagged releases and by manual workflow dispatch.
