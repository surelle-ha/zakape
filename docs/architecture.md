...
The desktop-only Godot Bridge adds a second narrow filesystem boundary for folders that contain a regular `project.godot` marker. Native commands canonicalize that root on every call, reject symbolic links and traversal, bound recursive indexing and payload sizes, and expose only project discovery, resource listing, supported sprite import, folder creation, conflict checks, and validated bundle writes. The Vue layer generates horizontal PNG sheets and Godot 4 `SpriteFrames` text resources from the existing render pipeline. Connected root paths persist as local preferences, but stale entries do not grant access and must pass native validation again. Browser and mobile builds render an explanatory capability state without project filesystem access. See `docs/godot-integration.md` for the artist workflow and limits.

At widths below 1024 px, the native shell removes desktop-only window chrome and minimum-size constraints. The desktop window can be resized down to 360 px wide for responsive testing. Tablets retain the vertical tool rail and Frames panel while moving the inspector into a right sheet. Phone portrait moves the tools into a horizontal dock and presents layers in a bottom sheet. The canvas uses Pointer Events for mouse, pen, and touch input; a fit control and the hand tool provide touch-friendly navigation. Safe-area insets protect controls around display cutouts and system navigation.

The model assistant is provider-neutral above its transport layer. Ollama, OpenAI-compatible endpoints, and the desktop-only Codex CLI adapter receive the same selected art skill, indexed project context, bounded PNG renders, and fixed operation schema. Generate, Animate, Inbetween, Restyle, Fix, and Extend specialize the prompt without changing editor authority. Before every pass, the Vue layer renders up to four current-frame composites with nearest-neighbor integer scaling. It applies each validated draft to an in-memory clone, renders that result again for actual visual review, and combines at most three incremental passes into one proposal and undo checkpoint.

Rust owns narrow native transports for local Ollama and Codex CLI. Ollama commands accept only loopback URLs, append fixed routes, refuse redirects and proxies, and enforce message and image bounds. The Codex adapter locates only a native `codex` executable and runs it without a command shell in a fresh temporary directory. It ignores user configuration and rules; disables shell, subagent, web-search, plugin, dependency-installation, login-shell, and file image-viewer capabilities; uses an ephemeral read-only run with a fixed JSON schema; and kills passes after three minutes. Compatible endpoints connect directly from the browser/WebView, require HTTPS unless they are on loopback, refuse credential-bearing URLs and redirects, and keep API keys in memory. Per-project chat transcripts persist in PGlite preferences; provider secrets and rendered review images do not. See `docs/ai/model-assistant.md` for provider setup and limits.

## Persistence

Home is the first interactive surface after the launch splash; the project launcher stays hidden until requested. On desktop, Rust creates the operating system's `Documents/zakape` directory and mirrors each autosaved project to `project_id.zakape`. Android uses the app-private data directory to avoid broad storage permissions and scoped-storage failures. The native commands accept project IDs rather than arbitrary paths, reject traversal characters, validate the project version and JSON identity, and cap project files at 32 MB. PGlite keeps the same project JSON as an indexed local cache and powers browser-mode persistence. The launcher merges both indexes for migration compatibility.
...
Type checks, unit tests, static generation, Rust formatting/lints, Playwright journeys, and UI snapshots run locally and in GitHub Actions. Interaction coverage includes the permanent Home tab, document tabs, the custom color mixer, guarded project/application closing, independent layer creation/visibility/renaming, undoable frame reordering, line/rectangle/circle previews, selection resize/rotation, frame-local menus, onion skinning, timeline collapse and preview-owned delay, hand panning, zoom-linked work grids, pointer-anchored wheel zoom, custom tooltips, command shortcuts, the first-project tour, assistant chat/scope/review, context-menu suppression, and browser-shortcut hardening. Native release bundles are produced on tagged releases and by manual workflow dispatch; a dedicated release job builds, verifies, renames, and attaches the matching Android APK before publication.
